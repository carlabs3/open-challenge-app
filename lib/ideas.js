 ...
 *
 * Ordre : les idées OUVERTES et CONSTITUÉES gardent l'ordre d'insertion (createdAt)
 * à l'intérieur d'un même défi. (Le tri des ouvertes par places libres est ajouté
 * séparément — un seul point à changer ici.)
 */
export async function buildIdeaViews(ideas, me, { withChallenge = false, challOrder = new Map() } = {}) {
  ...
  // Ordre inter-défis d'abord, puis insertion. (Le tri des ouvertes par places
  // libres viendra remplacer le second critère, ici uniquement.)
  openRaw.sort((a, b) => idx(a) - idx(b) || ins(a) - ins(b));
