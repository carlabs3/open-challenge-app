...
const participantSchema = new Schema(
  {
    ...
    // Statut auto-déclaré, éditable UNIQUEMENT tant que la personne n'a pas
    // d'équipe (ideaId null). Une fois dans une équipe, le statut affiché
    // « a déjà une équipe » est dérivé de ideaId, pas de ce champ.
    chercheEquipe: { type: String, enum: ["cherche", "idee"], default: "cherche" },
...
participantSchema.methods.toMe = function () {
  return {
    ...
    chercheEquipe: this.chercheEquipe,
    ideaId: this.ideaId,
    role: this.role,
...
const ideaSchema = new Schema(
  {
    ...
    // `confirmed` reflète la vérification de l'adresse mail du coordinateur, PAS une
    // action d'ouverture d'idée. Le champ est conservé mais n'a plus aucun usage dans
    // l'interface (pastille « à confirmer » retirée, cf. IdeaCard.js et API.md) : aucun
    // bouton ne le change, l'afficher promettait une action inexistante.
...
const joinRequestSchema = new Schema(
  {
    ...
    // Qui a envoyé l'invitation : n'importe quel membre de l'équipe, pas forcément
    // le coordinateur. `coord` reste la clé de permission ; `proposedBy` sert à
    // dire « c'est X qui vous a invité » et à prévenir X de la réponse.
    proposedBy: { type: Types.ObjectId, ref: "Participant" },
    note: String,
